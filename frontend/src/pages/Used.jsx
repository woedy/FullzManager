...
import { Search, MapPin, Phone, Eye, CheckCircle2 } from 'lucide-react';
...
const Used = () => {
    ...
    const fetchPeople = async (searchTerm = '', pageNum = 1) => {
        setLoading(true);
        try {
            const params = { search: searchTerm, page: pageNum, status: 'used' };
    ...
    const getDuration = (initiatedAt, completedAt) => {
        if (!initiatedAt || !completedAt) return 'Unknown';
        ...
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {people.map((person) => (
                        <div key={person.id} className="bg-slate-800 border border-green-700 rounded-xl p-5 hover:border-green-600 transition-colors shadow-lg opacity-75">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <div className="px-2 py-1 rounded-full text-xs font-medium bg-green-900 text-green-300 border border-green-700">
                                    USED
                                </div>
                            </div>

                            {/* Time Tracking Info */}
                            <div className="mb-4 bg-green-900/20 border border-green-700/50 rounded-lg p-3 space-y-1">
                                {person.initiated_at && (
                                    <div className="text-xs text-slate-400">
                                        <span className="font-medium text-slate-300">Started:</span> {new Date(person.initiated_at).toLocaleString()}
                                    </div>
                                )}
                                {person.completed_at && (
                                    <div className="text-xs text-slate-400">
                                        <span className="font-medium text-slate-300">Completed:</span> {new Date(person.completed_at).toLocaleString()}
                                    </div>
                                )}
                                <div className="flex items-center gap-2 text-green-300 text-sm mt-2">
                                    <span className="font-medium">Duration:</span>
                                    <span className="font-bold">{getDuration(person.initiated_at, person.completed_at)}</span>
                                </div>
                            </div>

                            {person.research_notes && (
                                <div className="mb-4 bg-slate-900/50 border border-slate-700/50 rounded-lg p-3">
                                    <label className="text-xs text-slate-500 font-medium mb-1 block">Research Notes</label>
                                    <p className="text-sm text-slate-300 whitespace-pre-wrap">{person.research_notes}</p>
                                </div>
                            )}
                            ...
                            <div className="flex border-t border-slate-700 pt-4 gap-2">
                                <Link to={`/person/${person.id}`} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white py-2 rounded text-center text-sm font-medium flex items-center justify-center gap-2">
                    ...
                    {people.length === 0 && (
                        <div className="col-span-full text-center py-12 text-slate-500">
                            No used fullz yet. Mark fullz as used from the "In Action" page.
                        </div>
                    )}
                </div>
            )}
